...
# ---------- Compact avatar CSS (sidebar) ----------
st.markdown(
    """
    <style>
      .avatar-img {
        max-height: 150px;   /* adjust to taste: 120–180px are good */
        width: auto;         /* preserve aspect ratio */
        border-radius: 12px; /* soft corners */
        display: block;
        margin: 6px auto 10px auto; /* center in sidebar */
      }
      .avatar-caption {
        text-align: center;
        font-size: 0.85rem;
...
def extract_slide_number(path: str) -> int:
    """Pull first number from filename. If none, return 9999 so it sorts last."""
        ...
        if t1.button("▶️ Play audio narration", key=f"tts_{slide_num}", use_container_width=True):
            if client is None:
                ...
                if slide_num not in st.session_state.tts_cache:
                    try:
                        # OpenAI TTS (no 'format' arg; defaults to MP3). Adjust model/voice if desired.
